  ...
  return (
    <div className="mx-auto min-w-0 max-w-7xl space-y-4 sm:space-y-6">
      ...
      <MetricGrid>
        ...
        <StatCard label={<AccentText>Online Order ROAS</AccentText>} value={ratios.roas === null ? "Unavailable" : `${ratios.roas.toFixed(2)}x`} helper={compare ? trendHelper(percentChange(ratios.roas, previousRatios.roas)) : undefined} state={status.error ? "error" : ratios.roas === null ? "empty" : "ready"} />
      ...
      <div className="grid gap-3 sm:gap-4 xl:grid-cols-2">
        ...
        <Card><h2 className="mb-3 text-base font-semibold sm:mb-4 sm:text-lg"><AccentText>Online Order ROAS</AccentText> over time</h2>{hasData ? <TrendChart data={daily} previousData={previousDaily} compare={compare} metric="roas" /> : <EmptyState />}</Card>
        ...
        <Card>
          ...
          <Table headers={["Channel", "Spend", "Revenue", "Est. total rev.", "Online orders", "Store visits", "Est. in-store", "Online Order ROAS", "Est. blended ROAS"]} rows={rows.map((item) => [
      ...
      <div className="grid gap-3 sm:gap-4 xl:grid-cols-2">
        <Card>
          ...
          <Table headers={["Campaign", "Platform", "Channel", "Spend", "Revenue", "Est. total rev.", "Online orders", "Est. in-store orders", "Online Order ROAS", "Est. blended ROAS"]} rows={campaignRows.map((item) => [
        ...
        <Card>
          <h2 className="mb-3 text-base font-semibold sm:mb-4 sm:text-lg">Top campaigns by <AccentText>Online Order ROAS</AccentText></h2>
          <Table headers={["Campaign", "Spend", "Revenue", "Est. total rev.", "Online orders", "Est. in-store orders", "Online Order ROAS", "Est. blended ROAS"]} rows={topRoasRows.map((item) => [
            item.campaign_name ?? item.campaign_id,
            currency.format(item.spend),
            currency.format(item.onlineRevenue),
            item.estimatedTotalRevenue === null ? "Unavailable" : currency.format(item.estimatedTotalRevenue),
        ...
        <Card>
          ...
          <Table headers={["Campaign", "Spend", "Revenue", "Est. total rev.", "Online orders", "Est. in-store orders", "Online Order ROAS", "Est. blended ROAS"]} rows={topConversionRows.map((item) => [
            item.campaign_name ?? item.campaign_id,
            currency.format(item.spend),
            currency.format(item.onlineRevenue),
            item.estimatedTotalRevenue === null ? "Unavailable" : currency.format(item.estimatedTotalRevenue),
