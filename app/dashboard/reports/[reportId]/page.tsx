...
import { publishMonthlyReport, unpublishMonthlyReport, updateMonthlyReportCopy } from "../actions";
...
export default async function ReportDetailPage({ params }: PageProps) {
  ...
  const canEditReportCopy = isAdmin && profile.email.toLowerCase() === "ryanskonovaloff@gmail.com";
  ...
  return (
    <div className="report-print-surface mx-auto max-w-7xl space-y-4 sm:space-y-6">
      ...
      {report ? <ReportContent report={report} paidImpactRows={paidImpactRows} showStatus={isAdmin} canEditReportCopy={canEditReportCopy} /> : null}
    </div>
  );
}

function ReportContent({ report, paidImpactRows, showStatus, canEditReportCopy }: {
  report: MonthlyReport;
  paidImpactRows: DailyPerformance[];
  showStatus: boolean;
  canEditReportCopy: boolean;
}) {
...
function ReportCopyEditor({ report, executiveSummary, wins, watchouts, nextSteps }: {
  report: MonthlyReport;
  ...
  return (
    <details className="portal-print-hidden group">
      <summary className="ml-auto flex w-fit cursor-pointer list-none items-center justify-center rounded-lg border border-accent/50 bg-accent/15 px-4 py-2 text-sm font-semibold text-accent transition hover:border-accent hover:bg-accent/20 group-open:border-white/20 group-open:bg-white/[0.04] group-open:text-white/75">
          Edit
        </summary>
      <Card className="mt-3">
        <div className="mb-4 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-lg font-semibold"><AccentText>Edit</AccentText> report copy</h2>
            <p className="mt-1 text-sm text-white/50">Update the text generated by the monthly workflow, then save when finished.</p>
          </div>
        </div>
        <form action={updateMonthlyReportCopy} className="space-y-4">
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            <EditorTextarea
              label="Paid ads commentary"
              name="paid_ads_commentary"
              defaultValue={report.paid_ads_commentary ?? ""}
              rows={5}
            />
            <EditorTextarea
              label="SEO commentary"
              name="seo_commentary"
              defaultValue={report.seo_commentary ?? ""}
              rows={5}
            />
            <EditorTextarea
              label="Month-over-month commentary"
              name="mom_commentary"
              defaultValue={report.mom_commentary ?? ""}
              rows={5}
            />
          </div>
          <div className="grid gap-4 lg:grid-cols-3">
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              className="inline-flex items-center justify-center rounded-lg border border-accent/50 bg-accent px-4 py-2 text-sm font-semibold text-black transition hover:bg-accent/90"
            >
              Save
            </button>
          </div>
...
function EditorTextarea({ label, helper, name, defaultValue, rows }: {
  label: string;
  helper?: string;
  name: string;
  defaultValue: string;
  ...
  return (
    <label className="block">
      ...
      {helper ? <span className="ml-2 text-xs text-white/35">{helper}</span> : null}
      <textarea
        name={name}
        defaultValue={defaultValue}
        rows={rows}
        className="mt-2 w-full resize-y rounded-lg border border-white/10 bg-black/25 px-3 py-2 text-sm leading-6 text-white/75 outline-none transition focus:border-accent/60"
      />
    </label>
  );
}
...
function SummarySection({ title, unavailable, summary, commentary, metrics }: {
  ...
  const highlights = summary && title !== "Month-over-Month" ? summaryHighlights(summary) : [];
...
function hasMomData(report: MonthlyReport) {
  if (momMetrics(report).length > 0) return true;
...
function momMetrics(report: MonthlyReport) {
  const summary = report.mom_summary;
  if (!summary) return [];
  const paid = readObject(summary, "paid_ads") ?? summary;
  const paidChange = { ...(readObject(summary, "paid_ads_change") ?? {}), ...paidAbsoluteChanges(report) };
  const seo = readObject(summary, "seo") ?? summary;
  return compactMetrics([
    ["Spend MoM", formatCurrencyChange(readNumber(paidChange, "spend", "spend_change"))],
    ["Online order revenue MoM", formatCurrencyChange(readNumber(paidChange, "revenue", "revenue_change"))],
    ["Estimated total revenue MoM", formatCurrencyChange(readNumber(paidChange, "estimated_total_revenue", "estimated_total_revenue_change"))],
    ...
    ["Online Order ROAS MoM", formatMultiplierChange(readNumber(paidChange, "roas", "roas_change", "online_order_roas"))],
    ["Blended ROAS MoM", formatMultiplierChange(readNumber(paidChange, "estimated_blended_roas", "estimated_blended_roas_change", "blended_roas"))],
...
function paidAbsoluteChanges(report: MonthlyReport) {
  const current = paidCurrent(report);
  const previous = report.paid_ads_summary ? readObject(report.paid_ads_summary, "previous") : null;
  if (!current || !previous) return {};

  const diff = (...keys: string[]) => {
    const currentValue = readNumber(current, ...keys);
    const previousValue = readNumber(previous, ...keys);
    return currentValue === null || previousValue === null ? null : currentValue - previousValue;
  };

  return {
    spend: diff("spend", "total_spend"),
    revenue: diff("revenue", "conversion_value", "total_revenue"),
    estimated_total_revenue: diff("estimated_total_revenue", "estimatedTotalRevenue"),
    estimated_in_store_revenue: diff("estimated_in_store_revenue", "estimatedInStoreRevenue"),
    roas: diff("roas"),
    estimated_blended_roas: diff("estimated_blended_roas", "estimatedBlendedRoas")
  };
}
...
function formatCurrencyChange(value: number | null) {
  if (value === null) return "Not available";
  const amount = currency.format(Math.abs(value));
  return `${value > 0 ? "+" : value < 0 ? "-" : ""}${amount}`;
}

function formatMultiplierChange(value: number | null) {
  if (value === null) return "Not available";
