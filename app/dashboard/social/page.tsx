...
export default async function SocialPage({ searchParams }: PageProps) {
  ...
  const { client, range, account, daily, content, paidRows, totals, previousTotals, status, lastUpdatedAt } = await getInstagramDashboardData(params?.range, params?.start, params?.end);
